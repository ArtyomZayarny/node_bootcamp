...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  // disable password confirmation that not need for this action
  await user.save({ validateBeforeSave: false });

  //send it(token) back as email
  ...
  try {
    ...
    //set reset fields to undefined and save user
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  // hased token from params
  const hashedToken = await crypto
    .createHash('sha256')
    .update(req.params.token)
    .digest('hex');

  // Get user base on token  check if passwordReset not expired
  ...
  //Set new password only if token not expires and user -ok
